...
しかも、機器を並べただけではなく、OSPF、eBGP、VRRP、DMVPN、FortiGateの設定から端末間の疎通確認までできました。
...
途中で構成図との違いや、機器設定が入っていない部分も見つかりましたが、その都度Codexへ伝えて修正しました。
...
- VLAN 10、20、30、40
- FortiGateによるインターネット接続

さらに、Codexは次のファイルも作成しました。

- `.gns3`プロジェクトファイル
- Cisco IOSのstartup-config
- VPCSのIPアドレス設定
- FortiGate用のCLI設定
- Cisco IOSへ設定を投入するPythonスクリプト
- FortiGateへ設定を投入するPythonスクリプト
...
なお、Cisco IOSやFortiGateのイメージそのものをCodexが用意するわけではありません。利用権のあるイメージを、自分のGNS3環境へ登録しておく必要があります。

今回は、GNS3 VMのアドレスを`192.168.100.102`へ統一し、登録済みのCisco IOSとFortiGate 7.0.9を使用しました。
...
| 用途           | ネットワーク      | 主なアドレス                               |
| -------------- | ----------------- | ------------------------------------------ |
| インターネット | `198.51.100.0/24` | FortiGate `.10`、試験端末 `.1`             |
...
| VLAN 10        | `10.10.10.0/24`   | L3SW10 `.1`、FortiGate `.254`              |
...
最初にプロジェクトを開いたとき、トポロジは作成されていましたが、Cisco IOSのコンソールを開くと`Router>`の状態でした。

### 設定されていない！！！

そこで、Codexに全ネットワーク機器の状態を確認してもらい、startup-configをコンソール経由で投入するスクリプトを作成しました。

```bash
python3 configure_cisco_ios.py
```
...
設定投入後には`write memory`まで実行します。

FortiGateについても、初回起動後に設定を自動投入できるようにしました。

```bash
python3 configure_fortigate.py
```

既に管理者パスワードを変更している場合は、次のように再投入できます。

```bash
python3 configure_fortigate.py --password '<設定済みパスワード>'
```

ラボ用とはいえ、記事やGitHubへ実際のパスワードを載せないように注意しましょう。
...
今回、ネットワーク構成図の写真から26ノード、27リンクのGNS3プロジェクトを作成し、各ネットワーク機器への設定投入と端末間の疎通確認までできました。
